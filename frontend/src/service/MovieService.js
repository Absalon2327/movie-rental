...
const MovieService = {
  ...
  createMovie: async (movieData) => {
    ...
    Object.keys(movieData).forEach((key) => {
      // Verificar si es el campo de imagen
      if (key === "image" && movieData[key] instanceof File) {
        formData.append(key, movieData[key]); // Agregar el archivo de imagen directamente
      } else {
        formData.append(key, movieData[key]); // Agregar otros campos normalmente
      }
    });
